...
final class HorizontalTileLayoutTests: XCTestCase {
	func testStandardSquareShouldBeAbleToContainFourMinimumSquareSize() {
		let blocks: [HorizontalTileLayout.BlockType] = [.block, .block, .block]
		let sut = HorizontalTileLayout(blocks: blocks)
	...
	func test_shouldCalculateSizeOfTheContainerCorrectly_whenTheNumberOfTilesIsLessOrEqual4Double() {
		var blocks: [HorizontalTileLayout.BlockType] = [.block, .block, .block]
		let sut = HorizontalTileLayout(blocks: blocks)
		let minimumSquare: CGSize = .init(width: 100, height: 100)
	...
	func test_shouldCalculateSizeOfTheContainerCorrectly_whenTheNumberOfTilesIsOdd() {
		let blocks: [HorizontalTileLayout.BlockType] = [.block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block]
		let sut = HorizontalTileLayout(blocks: blocks)
		let minimumSquare: CGSize = .init(width: 100, height: 100)
		var cache: HorizontalTileLayout.Cache = (standardSquare: sut.standardSquareSize(from: minimumSquare), minimumSquare: minimumSquare)
	...
	func test_shouldCalculateSizeOfTheContainerCorrectly_whenTheNumberOfTilesIsEvenButNotDevidableBy4() {
		let blocks: [HorizontalTileLayout.BlockType] = [.block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block, .block]
		let sut = HorizontalTileLayout(blocks: blocks)
		let minimumSquare: CGSize = .init(width: 100, height: 100)
		var cache: HorizontalTileLayout.Cache = (standardSquare: sut.standardSquareSize(from: minimumSquare), minimumSquare: minimumSquare)
		let size = sut.sizeThatFitsBlocks(proposal: .init(width: 100, height: 100), blocks: blocks, cache: &cache)
	...
	func test_shouldCalculateSizeOfTheContainerCorrectly_whenblocksContainsOnlySingleStandard() {
		let blocks: [HorizontalTileLayout.BlockType] = [.full, .full, .full, .full, .full, .full, .full, .full, .full, .full, .full, .full]
		let sut = HorizontalTileLayout(blocks: blocks)
		let minimumSquare: CGSize = .init(width: 100, height: 100)
		var cache: HorizontalTileLayout.Cache = (standardSquare: sut.standardSquareSize(from: minimumSquare), minimumSquare: minimumSquare)
		let size = sut.sizeThatFitsBlocks(proposal: .init(width: 100, height: 100), blocks: blocks, cache: &cache)
	...
	func test_shouldCalculateSizeOfTheContainerCorrectly_whenblocksContainsOnlyFullWidth() {
		...
		let blocks: [HorizontalTileLayout.BlockType] = tileWidth.map({.fullCustom(width: $0)})
		let sut = HorizontalTileLayout(blocks: blocks)
		let minimumSquare: CGSize = .init(width: 100, height: 100)
		var cache: HorizontalTileLayout.Cache = (standardSquare: sut.standardSquareSize(from: minimumSquare), minimumSquare: minimumSquare)
		let size = sut.sizeThatFitsBlocks(proposal: .init(width: 100, height: 100), blocks: blocks, cache: &cache)
	...
	func test_shouldCalculateSizeOfTheContainerCorrectly_whenblocksContainsMixOfTiles() {
		let blocks: [HorizontalTileLayout.BlockType] = [.fullCustom(width: 100), .full, .full, .block, .block, .block, .full, .fullCustom(width: 430),.fullCustom(width: 540), .block, .block, .full,.block, .full, .block, .block, .block, .block,]
		let sut = HorizontalTileLayout(blocks: blocks)
		let minimumSquare: CGSize = .init(width: 100, height: 100)
		var cache: HorizontalTileLayout.Cache = (standardSquare: sut.standardSquareSize(from: minimumSquare), minimumSquare: minimumSquare)
		let size = sut.sizeThatFitsBlocks(proposal: .init(width: 100, height: 100), blocks: blocks, cache: &cache)
	...
	func test_shouldPlaceTheItemsCorrectly_whenOnlySquareItemsInside() {
		...
		let blocks: [HorizontalTileLayout.BlockType] = tileWidth.map({_ in .full})
		let sut = HorizontalTileLayout(blocks: blocks)
		let minimumSquare: CGSize = .init(width: 100, height: 100)
		var cache: HorizontalTileLayout.Cache = (standardSquare: sut.standardSquareSize(from: minimumSquare), minimumSquare: minimumSquare)
		let size = sut.sizeThatFitsBlocks(proposal: .init(width: 100, height: 100), blocks: blocks, cache: &cache)
	...
	func test_shouldPlaceTheItemsCorrectly_whenOnlyDoubleItemsInside() {
		...
		let blocks: [HorizontalTileLayout.BlockType] = tileWidth.map({_ in .block})
		let sut = HorizontalTileLayout(blocks: blocks)
	...
	func test_shouldPlaceTheItemsCorrectly_whenAMixOfTileItemsInside() {
		...
		let blocks: [HorizontalTileLayout.BlockType] = tileWidth.map({_ in .block}) + [.full, .fullCustom(width: 200), .block, .fullCustom(width: 242)]
		let sut = HorizontalTileLayout(blocks: blocks)
